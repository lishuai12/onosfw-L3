...
public class FloatingIpWebResource extends AbstractWebResource {
    ...
    public Response listFloatingIps() {
        Iterable<FloatingIp> floatingIps = get(FloatingIpService.class)
                .getFloatingIps();
    ...
    public Response getFloatingIp(@PathParam("floatingIpUUID") String id,
        ...
        if (!get(FloatingIpService.class).exists(FloatingIpId.floatingIpId(id))) {
            return Response.status(NOT_FOUND).entity(NOT_EXIST).build();
        }
        FloatingIp sub = nullIsNotFound(get(FloatingIpService.class)
                .getFloatingIp(FloatingIpId.floatingIpId(id)), GET_FAIL);
    ...
    public Response createFloatingIp(final InputStream input) {
        ...
        try {
            ...
            Iterable<FloatingIp> floatingIps = createOrUpdateByInputStream(subnode);
            Boolean result = nullIsNotFound((get(FloatingIpService.class)
                                                    .createFloatingIp(floatingIps)),
                                            CREATE_FAIL);
            if (!result) {
    ...
    public Response updateFloatingIp(@PathParam("floatingIpUUID") String id,
        ...
        try {
            ...
            Iterable<FloatingIp> floatingIps = createOrUpdateByInputStream(subnode);
            Boolean result = nullIsNotFound(get(FloatingIpService.class)
                    .updateFloatingIp(floatingIps), UPDATE_FAIL);
            if (!result) {
    ...
    public Response deleteSingleFloatingIp(@PathParam("floatingIpUUID") String id)
            throws IOException {
        try {
            FloatingIpId floatingIpId = FloatingIpId.floatingIpId(id);
            Set<FloatingIpId> floatingIpIds = Sets.newHashSet(floatingIpId);
            get(FloatingIpService.class).removeFloatingIp(floatingIpIds);
    ...
    private Iterable<FloatingIp> createOrUpdateByInputStream(JsonNode subnode)
            throws Exception {
        checkNotNull(subnode, JSON_NOT_NULL);
        Iterable<FloatingIp> floatingIps = null;
        ...
        if (floatingIpNodes.isArray()) {
            throw new BadRequestExecption("only singleton requests allowed");
        } else {
     ...
     * @throws Exception
     */
    public Iterable<FloatingIp> changeJsonToSub(JsonNode floatingIpNodes)
            throws Exception {
        ...
        FloatingIpId id = FloatingIpId.floatingIpId(floatingIpNodes.get("id")
                .asText());
        ...
        if (floatingIpNodes.hasNonNull("router_id")) {
            routerId = RouterId.routerId(floatingIpNodes.get("router_id")
                    .asText());
        }

//        if (!floatingIpNodes.hasNonNull("fixed_ip_address")) {
//            throw new BadRequestExecption("fixed_ip_address should not be null");
//        } else if (floatingIpNodes.get("fixed_ip_address").asText().isEmpty()) {
//            throw new BadRequestExecption(
//                                          "fixed_ip_address should not be empty");
//        }
        ...
        FloatIpStatus status = FloatIpStatus.valueOf(floatingIpNodes
                .get("status").asText());
