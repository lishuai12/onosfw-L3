...
public class FlowEntryBuilder {
    ...
    private static final int NXAST_REG_MOVE = 6;
    private static final int NXAST_REG_LOAD = 7;
        ...
        for (OFAction act : actions) {
            switch (act.getType()) {
                ...
                if (exp.getExperimenter() == 0x80005A06
                    ...
                    short lambda = ((OFOxmOchSigidBasic) ct.getField())
                            .getValue().getChannelNumber();
                    builder.add(Instructions
                            .modL0Lambda(Lambda.indexedLambda(lambda)));
                }  else if (exp.getExperimenter() == 0x2320) {
                    OFActionNicira nicira = (OFActionNicira) exp;
                    if (nicira.getSubtype() == NXAST_REG_MOVE) {
                        OFActionNiciraMove move = (OFActionNiciraMove) nicira;
                        if (move.getSrc() == 0x00000e04 && move.getDst() == 0x00001004) {
                            builder.add(Instructions.moveNxmIpSrcToNxmIpDst());
                        } else if (move.getSrc() == 0x00002004 && move.getDst() == 0x00002204) {
                            builder.add(Instructions.moveNxmArpSpaToNxmArpTpa());
                        } else if (move.getSrc() == 0x00012206 && move.getDst() == 0x00012406) {
                            builder.add(Instructions.moveNxmArpShaToNxmArpTha());
                        } else if (move.getSrc() == 0x00000406 && move.getDst() == 0x00000206) {
                            builder.add(Instructions.moveNxmEthSrcToNxmEthDst());
                        }
                    } else if (nicira.getSubtype() == NXAST_REG_LOAD) {
                        OFActionNiciraLoad load = (OFActionNiciraLoad) nicira;
                        if (load.getDst() == 0x00012206) {
                            builder.add(Instructions.loadNxmArpSha(MacAddress.valueOf(load.getValue().getValue())));
                        } else if (load.getDst() == 0x00002004) {
                            int value = Integer.valueOf(((Long) load.getValue().getValue()).toString());
                            builder.add(Instructions.loadNxmArpSpa(Ip4Address.valueOf(value)));
                        } else if (load.getDst() == 0x00001e02) {
                            builder.add(Instructions.loadNxmArpOp(load.getValue().getValue()));
                        }
                    }
                } else {
                    log.warn("Unsupported OFActionExperimenter {}",
                             exp.getExperimenter());
                }
                break;
            case SET_FIELD:
    ...
    private TrafficSelector buildSelector() {
        ...
        for (MatchField<?> field : match.getMatchFields()) {
            ...
            case TUNNEL_ID:
                ...
                break;
            case ARP_TPA:
                if (match.isPartiallyMasked(MatchField.ARP_TPA)) {
                    Masked<IPv4Address> maskedIp = match.getMasked(MatchField.ARP_TPA);
                    ip4Prefix = Ip4Prefix.valueOf(
                            maskedIp.getValue().getInt(),
                            maskedIp.getMask().asCidrMaskLength());
                } else {
                    ip4Prefix = Ip4Prefix.valueOf(
                            match.get(MatchField.ARP_TPA).getInt(),
                            Ip4Prefix.MAX_MASK_LENGTH);
                }
                builder.matchArpTpa(ip4Prefix);
                break;
            case ARP_OP:
            case ARP_SHA:
            case ARP_SPA:
