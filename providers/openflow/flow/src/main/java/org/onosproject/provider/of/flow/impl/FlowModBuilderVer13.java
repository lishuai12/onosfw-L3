...
import org.onosproject.net.flow.instructions.LoadInstruction;
...
import org.onosproject.net.flow.instructions.MoveInstruction;
...
public class FlowModBuilderVer13 extends FlowModBuilder {
    ...
    private List<OFAction> buildActions(List<Instruction> treatments) {
        ...
        for (Instruction i : treatments) {
            switch (i.type()) {
                ...
                break;
            case MOVE:
                actions.add(buildMoveAction(i));
                break;
            case LOAD:
                actions.add(buildLoadAction(i));
                    break;
                default:
    ...
    /**
     * Builds a move action.
     *
     * @param i a instruction
     * @return ofaction a move ofaction
     */
    private OFAction buildMoveAction(Instruction i) {
        MoveInstruction mi = (MoveInstruction) i;
        OFActionNiciraMove.Builder action = factory().actions()
                .buildNiciraMove();
        switch (mi.subtype()) {
        case NXMARPSHATONXMARPTHA:
            action.setDstOfs(Integer.valueOf(0));
            action.setSrcOfs(Integer.valueOf(0));
            action.setNBits(48);
            action.setSrc(0x00012206);
            action.setDst(0x00012406);
            break;
        case NXMETHSRCTONXMETHDST:
            action.setDstOfs(Integer.valueOf(0));
            action.setSrcOfs(Integer.valueOf(0));
            action.setNBits(48);
            action.setSrc(0x00000406);
            action.setDst(0x00000206);
            break;
        case NXMARPSPATONXMARPTPA:
            action.setDstOfs(Integer.valueOf(0));
            action.setSrcOfs(Integer.valueOf(0));
            action.setNBits(32);
            action.setSrc(0x00002004);
            action.setDst(0x00002204);
            break;
        case NXMIPSRCTONXMIPDST:
            action.setDstOfs(Integer.valueOf(0));
            action.setSrcOfs(Integer.valueOf(0));
            action.setNBits(32);
            action.setSrc(0x00000e04);
            action.setDst(0x00001004);
            break;
        default:
            log.warn("Unimplemented action type {}.", mi.subtype());
            break;
        }
    ...
    /**
     * Builds a load action.
     *
     * @param i a instruction
     * @return ofaction a load ofaction
     */
    private OFAction buildLoadAction(Instruction i) {
        ...
        switch (li.subtype()) {
        case NXMARPSHA:
            NxmArpShaInstruction si = (NxmArpShaInstruction) li;
            oxm = factory().oxms().arpSha(MacAddress.of(si.mac().toLong()));
            break;
        case NXMARPSPA:
            NxmArpSpaInstruction ai = (NxmArpSpaInstruction) li;
            ip4 = ai.ip().getIp4Address();
            oxm = factory().oxms().arpSpa(IPv4Address.of(ip4.toInt()));
            break;
        case NXMARPOP:
            NxmArpOpInstruction oi = (NxmArpOpInstruction) li;
            oxm = factory().oxms().arpOp(ArpOpcode.of((int) oi.op()));
            break;
        default:
            log.warn("Unimplemented action type {}.", li.subtype());
            break;
        }
